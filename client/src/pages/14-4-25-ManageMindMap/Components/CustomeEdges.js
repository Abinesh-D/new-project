...
const DashedEdge = ({ edgePath, stroke }) => {
...
const PastedEdgeSVG = ({ paths, direction = "left" }) => {
  const isRight = direction === "right";

  return (
    <svg width="100%" height="100%" viewBox="0 -227 124 300">
      <g transform="translate(-20, 0)">
        {paths.map(({ nodeId, fill, transformY, stroke, curve }) => {
          const translateGroupX = isRight ? 250.0027 : -250.0027;
          const arrowPath = "M 0 -4.2264 6.4057 0 0 4.2264 6.4057 4.2264 6.4057 -4.2264 z";
          const arrowTransform = isRight
            ? `translate(-6.4057, 0)`
            : `translate(6.4057, 0) scale(-1, 1)`;

          const sideConnectorTransform = isRight
            ? `translate(100.6043, ${transformY}) rotate(180)`
            : `translate(-100.6043, ${transformY})`;

          return (
            <g key={nodeId} transform={`translate(${translateGroupX}, 0)`}>
              <path d={arrowPath} transform={arrowTransform} fill={fill} />
              <path
                d="M 0.2 -4.0764 -0.1 -4.0764 -6.1808 0 -0.1 4.0764 0.2 4.0764"
                transform={sideConnectorTransform}
                fill={fill}
              />
              <path
                fill="none"
...
const CustomEdge = ({ id, data, sourceX, sourceY, targetX, targetY }) => {
  ...
  const { isFromExcel, isImageNode, pastedEdge, direction , offsetX = 0 } = data || {};

  if (isFromExcel || isImageNode) {
    const adjustedTargetX = targetX + offsetX;
    const edgePath = `M ${sourceX},${sourceY} C ${sourceX + (adjustedTargetX - sourceX) * 0.3},${sourceY} ${sourceX + (adjustedTargetX - sourceX) * 0.7},${targetY} ${adjustedTargetX},${targetY}`;
    return (
      <g className="custom-d3-edge">
        <DashedEdge edgePath={edgePath} stroke={data?.color || "#ebd95f"} />
      </g>
    );
  }
  ...
  if (pastedEdge) {
    ...
    const paths = [...Array(data.pastedCount).keys()].map(i => {
      const yOffset = i * 32;
      const fillColor = edgeColors[i % edgeColors.length];
  
      const curve = isRight
        ? `M 1 0 C ${70 + i * 2.56} 0 ${50 + i * 10} ${yOffset} 100.5943 ${yOffset}`
        : `M 1 0 C ${-70 - i * 2.56} 0 ${-50 + i * 10} ${yOffset} -100.5943 ${yOffset}`;
    ...
    return <PastedEdgeSVG paths={paths} direction={data?.direction} />;
  }
  const config = directionConfigs[direction]?.[0] || {};
  return (
    <svg width="100%" height="100%" viewBox="0 -227 124 300">
  <g key={id} className="node-connection" transform="translate(100, 100)">
    <path d="M -4.595 0 0 -6.9643 4.595 0 4.595 -6.9643 -4.595 -6.9643 z" transform={config.transform} fill={strokeColor} />
    <path d="M -0.2 -4.4450 0.1 -4.4450 6.7396 0 0.1 4.4450 -0.2 4.4450" transform={config.arrowTransform} fill={strokeColor} />
    <path fill="none" strokeLinejoin="round" strokeWidth="9.17" d={config.pathD} stroke={strokeColor} />
  </g>
</svg>
...
// const PastedEdgeSVG = ({ paths }) => (
//   <svg width="100%" height="100%" viewBox="0 -227 124 300">
//     <g transform="translate(-20, 0)">
//       {paths.map(({ nodeId, fill, transformY, stroke, curve }) => (
//         <g key={nodeId} transform="translate(-250.0027,0.0000)">
//           <path d="M 0 -4.2264 6.4057 0 0 4.2264 6.4057 4.2264 6.4057 -4.2264 z" transform="translate(6.4057,0) scale(-1, 1)" fill={fill} />
//           <path d="M 0.2 -4.0764 -0.1 -4.0764 -6.1808 0 -0.1 4.0764 0.2 4.0764" transform={`translate(-100.6043,${transformY})`} fill={fill} />
//           <path fill="none" strokeLinejoin="round" strokeWidth="8.4529" d={curve} stroke={stroke} />
//         </g>
//       ))}
//     </g>
//   </svg>
// );
  ...
  //     return {
  //       nodeId: `f${i + 1}`,
  //       fill: edgeColors[i % edgeColors.length],
  //       transformY: yOffset,
  //       stroke: edgeColors[i % edgeColors.length],
  //       curve: `M 1 0 C ${-70 - i * 2.56} 0 ${-50 + i * 10} ${yOffset} -100.5943 ${yOffset}`,
  //     };
  //   });
  //   return <PastedEdgeSVG paths={paths} />;
  // }
...
//   const isImageNode = data?.isImageNode;
//   const pastedEdge = data?.pastedEdge;

//   useEffect(() => {
//     if ((!isFromExcel && !isImageNode) || !pathRef.current) return;
//     const path = d3.select(pathRef.current);
//     path
//       .attr("stroke-dasharray", "10, 8")
//       .attr("stroke-dashoffset", "18")
...
//           .on("end", repeat);
//       });
//   }, [isFromExcel, isImageNode, sourceX, sourceY, targetX, targetY]);
...
//   if (pastedEdge) {
//     const paths = [
//       { nodeId: 'f1', fill: '#edd965', transformY: 0, stroke: '#edd965', curve: 'M 1 0 C -70.1160 0 -49.7971 0 -100.5943 0' },
//       { nodeId: 'f2', fill: '#ecd656', transformY: 32, stroke: '#ecd656', curve: 'M 1 0 C -72.6760 0 -40.1971 32 -100.5943 32' },
//       { nodeId: 'f3', fill: '#ebd869', transformY: 64, stroke: '#ebd869', curve: 'M 1 0 C -75.2360 0 -30.5971 64 -100.5943 64' },
//       { nodeId: 'f4', fill: '#ead662', transformY: 96, stroke: '#ead662', curve: 'M 1 0 C -77.7960 0 -20.9971 96 -100.5943 96' },
//       { nodeId: 'f5', fill: '#ead869', transformY: 128, stroke: '#ead869', curve: 'M 1 0 C -80.3560 0 -11.3971 128 -100.5943 128' },
//       { nodeId: 'f6', fill: '#ead456', transformY: 160, stroke: '#ead456', curve: 'M 1 0 C -82.9160 0 -1.7971 160 -100.5943 160' },
//       { nodeId: 'f7', fill: '#ecd75e', transformY: 192, stroke: '#ecd75e', curve: 'M 1 0 C -85.4760 0 7.8029 192 -100.5943 192' },
//     ];
...
//               <path
//                 d="M 0.2 -4.0764 -0.1 -4.0764 -6.1808 0 -0.1 4.0764 0.2 4.0764"
//                 transform={`translate(-100.6043,${transformY})`}
//                 fill={fill}
//               />
//               <path
//                 fill="none"
...
//       <g data-nodeid="167e323ef47a84113377e23a" className="node-connection" transform="translate(-20, 0)">
//         {[
//           {
//             nodeId: 'f438d2d74914561cdb1e2ab6',
//             fill: '#edd965',
//             transformY: 0,
//             stroke: '#edd965',
//             curve: 'M 1 0 C -70.1160 0 -49.7971 0 -100.5943 0'
//           },
//           {
//             nodeId: '1f3c8f39589d0915d58c48a7',
//             fill: '#ecd656',
//             transformY: 32,
//             stroke: '#ecd656',
//             curve: 'M 1 0 C -72.6760 0 -40.1971 32.0000 -100.5943 32.0000'
//           },
//           {
//             nodeId: 'b44c63d37c90c5c9db5a9b46',
//             fill: '#ebd869',
//             transformY: 64,
//             stroke: '#ebd869',
//             curve: 'M 1 0 C -75.2360 0 -30.5971 64 -100.5943 64'
//           },
//           {
//             nodeId: 'dc7157c875779e9b506e5f79',
//             fill: '#ead662',
//             transformY: 96,
//             stroke: '#ead662',
//             curve: 'M 1 0 C -77.7960 0 -20.9971 96 -100.5943 96'
//           },
//           {
//             nodeId: '92212893aab5495123e6c3a2',
//             fill: '#ead869',
//             transformY: 128,
//             stroke: '#ead869',
//             curve: 'M 1 0 C -80.3560 0 -11.3971 128 -100.5943 128'
//           },
//           {
//             nodeId: 'b5e7278c2aae9d1d584d2a01',
//             fill: '#ead456',
//             transformY: 160,
//             stroke: '#ead456',
//             curve: 'M 1 0 C -82.9160 0 -1.7971 160 -100.5943 160'
//           },
//           {
//             nodeId: '88af355c61eee947927b9d30',
//             fill: '#ecd75e',
//             transformY: 192,
//             stroke: '#ecd75e',
//             curve: 'M 1 0 C -85.4760 0 7.8029 192 -100.5943 192'
//           }
//         ]
//           .slice(0, data.pasteCount)
//           .map(({ nodeId, fill, transformY, stroke, curve }) => (
//             <g key={nodeId} data-nodeid={nodeId} className="node-connection" transform="translate(-250.0027,0.0000)">
//               <path
//                 d="M 0 -4.2264 6.4057 0 0 4.2264 6.4057 4.2264 6.4057 -4.2264 z"
//                 transform="translate(6.4057,0) scale(-1, 1)"
//                 fill={fill}
...
//           .slice(0, data.pasteCount)
//           .map(({ nodeId, fill, transformY, stroke, curve }) => (
//             <g key={nodeId} data-nodeid={nodeId} className="node-connection" transform="translate(250.0027,0.0000) scale(-1, 1)">
//               <path
//                 d="M 0 -4.2264 6.4057 0 0 4.2264 6.4057 4.2264 6.4057 -4.2264 z"
//                 transform="translate(6.4057,0) rotate(180)"
//                 fill={fill}
//                 stroke="none"
//               />
//               <path
...
//   // Static edge SVGs for pasted nodes
//   if (pastedEdge) {
//     const paths = [
//       { nodeId: 'f1', fill: '#edd965', transformY: 0, stroke: '#edd965', curve: 'M 1 0 C -70.1160 0 -49.7971 0 -100.5943 0' },
//       { nodeId: 'f2', fill: '#ecd656', transformY: 32, stroke: '#ecd656', curve: 'M 1 0 C -72.6760 0 -40.1971 32 -100.5943 32' },
...
//   // Regular directional edge (top/right/left/bottom)
//   const { direction = "right", index = 0 } = data || {};
//   const config = directionConfigs[direction]?.[index] || {};
...
// export default CustomEdge;
        ...
        {/* <g transform="translate(0, 0)">
          {dynamicPaths.map(({ nodeId, fill, stroke, transformY, curve }) => (
            <g key={nodeId} transform="translate(-250.0027,0.0000)">
              <path
                d="M 0 -4.2264 6.4057 0 0 4.2264 6.4057 4.2264 6.4057 -4.2264 z"
                transform="translate(6.4057,0) scale(-1, 1)"
...
// <g data-nodeid="167e323ef47a84113377e23a" className="node-connection" transform="translate(0, 0)">
//   {[
//     {
//       nodeId: 'f438d2d74914561cdb1e2ab6',
//       fill: '#edd965',
