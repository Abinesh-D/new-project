...
import "./MindMap.css";
import { fetchDefaultMindMap } from "./MindMapSlice/flowSlice";
...
import { setNodes, setEdges } from "./MindMapSlice/flowSlice";
import getUpdatedNodes from "./MindMapSlice/Reusable/UpdateNode";


const FlowChart = () => {
  const dispatch = useDispatch();
  const floData = useSelector((state) => state.flow);
  console.log(floData, 'FlowData',)

  const history = useRef([{ nodes: floData.nodes, edges: floData.edges }]);
  const redoStack = useRef([]);

  useEffect(() => {
    dispatch(fetchDefaultMindMap())
      .then((data) => {
        console.log("Default nodes/edges loaded", data);
      })
      .catch((err) => {
  ...
  const saveState = useCallback(() => {
    ...
    if (JSON.stringify(lastState) !== JSON.stringify({ nodes: floData.nodes, edges: floData.edges })) {
      history.current.push({ nodes: floData.nodes, edges: floData.edges });
      redoStack.current = [];
    }
  }, [floData]);
  ...
  const onNodesChange = useCallback((changes) => {
    ...
    const clonedNodes = cloneDeep(floData.nodes);
    const updatedNodes = applyNodeChanges(changes, clonedNodes);
    console.log('updatedNodes :>> ', updatedNodes);
    dispatch(setNodes(updatedNodes));
  }, [saveState, dispatch, floData.nodes]);

  const onLabelChange = useCallback((nodeId, newLabel) => {
    console.log('nodeId, newLabel :>> ', nodeId, newLabel);
    saveState();
    const updatedNodes = floData.nodes.map((node) =>
  ...
  const processedEdges = ProcessEdges(floData.edges);
  ...
  }, [floData, deleteNode, addNode, onLabelChange, handleFileUpload, dispatch]);
...
//   }, [dispatch]);
  ...
  // const onNodesChange = useCallback((changes) => {
  //   saveState();
  //   dispatch(setNodes((nds) => applyNodeChanges(changes, nds)));
  // }, [saveState, dispatch]);

//   const onLabelChange = useCallback((nodeId, newLabel) => {
//     saveState();
//     dispatch(setNodes((prevNodes) =>
//       prevNodes.map((node) =>
//         node.id === nodeId ? { ...node, data: { ...node.data, label: newLabel } } : node
//       )
//     ));
...
//   const deleteNode = DeleteNodeHandler({ setNodes: (nodes) => dispatch(setNodes(nodes)), setEdges: (edges) => dispatch(setEdges(edges)) });
//   const addNode = AddNodeHandler({
//     nodes,
//     setNodes: (nodes) => dispatch(setNodes(nodes)),
//     setEdges: (edges) => dispatch(setEdges(edges)),
