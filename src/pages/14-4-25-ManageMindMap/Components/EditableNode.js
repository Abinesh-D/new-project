...
import { updateNodeData } from "./MindMapSlice/flowSlice";
...
const EditableNode = ({ data, id }) => {
  console.log('EditableNode', data, id);
  const dispatch = useDispatch();

  const textAreaRef = useRef(null);

  const [label, setLabel] = useState(data.data.label);
  const [isBold, setIsBold] = useState(data.data.bold);
  const [isItalic, setIsItalic] = useState(data.data.italic);
  ...
  const handleUpdate = async () => {
    try {
      await updateNodeData(data._id, {
        label: data.data.label,
        link: data.data.link,
        bold: data.data.bold,
        italic: data.data.italic
      }
      );
    } catch (err) {
  ...
  const applyFormat = async (format) => {
    ...
    if (format === "bold") {
      updatedFields.bold = !data.data.bold;
    }
  
    if (format === "italic") {
      updatedFields.italic = !data.data.italic;
    }
  
    if (format === "link") {
      const newLink = prompt("Enter the link:", data.data.link || "");
      if (newLink) {
        updatedFields.link = newLink;
      } else {
        return;
      }
    }
    ...
    try {
      await updateNodeData(data._id, updatedFields);
    } catch (err) {
      console.error("Failed to update node formatting:", err);
    }
  };
  ...
  const editingContent = <TextEditor
    ...
    handleChange={(e) => handleChange(e,nodes._id) }
  ...
  return (
    <div
      className={!isEditing && data.id === '6616c2e8c5043c003ad731ae' ? "editable-node" : ""}
      {...(data.id === "6616c2e8c5043c003ad731ae" && {
...
//     // const labelTrue = dispatch(updateNodeLabel(nodeId, label));
//     onLabelChange(id, label);
//   };
...
//     label={label}
//     textAreaRef={textAreaRef}
//     handleChange={(e) => handleChange(e, data._id) }
//     handleKeyDown={handleKeyDown}
//     applyFormat={applyFormat}
//     handleFileUpload={handleFileUpload}
//     setNodes={setNodes}
...
//   return (
//     <div
//       className={!isEditing && data._id === '67f6c9a00e37d55a1ae21d09' ? "editable-node" : ""}
//       {...(data._id === "67f6c9a00e37d55a1ae21d09" && {
//         onMouseEnter: () => setIsHovered(true),
//         onMouseLeave: () => setIsHovered(false),
//       })}
//     >
...
// import TextEditor from "./MindMapSlice/Reusable/TextEditor";
// import { useDispatch } from "react-redux";
// import { updateNodeLabelAndStyles } from "./MindMapSlice/flowSlice";
...
//   const [isBold, setIsBold] = useState(data.textStyles?.isBold || false);
//   const [isItalic, setIsItalic] = useState(data.textStyles?.isItalic || false);
//   const [link, setLink] = useState(data.textStyles?.link || "");
//   const [isEditing, setIsEditing] = useState(false);
//   const [isHovered, setIsHovered] = useState(false);
...
//     if (data.label) {
//       setLabel(data.label)
//       setIsBold(data.textStyles?.isBold);
//       setIsItalic(data.textStyles?.isItalic);
//       setLink(data.textStyles?.link)
//     }
//   }, [label, isBold, isItalic, link])
...
//   const syncWithAPI = async (updatedLabel = label, customStyles = {}) => {
//     try {
//       const result = await updateNodeLabelAndStyles({
//         nodeId: nodes[0]._id,
//         label: updatedLabel,
//         isBold: customStyles.isBold ?? isBold,
//         isItalic: customStyles.isItalic ?? isItalic,
//         link: customStyles.link ?? link,
//       });
      
  
//       setLabel(result.label);
//       setIsBold(result.textStyles?.bold || false);
//       setIsItalic(result.textStyles?.italic || false);
//       setLink(result.textStyles?.link || "");
//     } catch (error) {
...
//       setIsBold(newBold);
//       syncWithAPI(label, { bold: newBold });
//     }
//     if (format === "italic") {
//       const newItalic = !isItalic;
//       setIsItalic(newItalic);
//       syncWithAPI(label, { italic: newItalic });
//     }
//     if (format === "link") {
//       const newLink = prompt("Enter the link:", link);
//       if (newLink !== null) {
//         setLink(newLink);
//         syncWithAPI(label, { link: newLink });
//       }
//     }
//   };
...
//   return (
//     <div
//       className={!isEditing && nodes[0].id === id ? "editable-node" : ""}
//       {...(nodes[0].id === id && {
//         onMouseEnter: () => setIsHovered(true),
//         onMouseLeave: () => setIsHovered(false),
//       })}
...
// import ViewContent from "./MindMapSlice/Reusable/ViewContent";
// import TextEditor from "./MindMapSlice/Reusable/TextEditor";
// import { updateNodeLabel } from "./MindMapSlice/flowSlice";
// import { useDispatch } from "react-redux";
...
//     label={label}
//     textAreaRef={textAreaRef}
//     handleChange={(e) => handleChange(e, data.nodes[0].id) }
//     handleKeyDown={handleKeyDown}
//     applyFormat={applyFormat}
//     handleFileUpload={handleFileUpload}
//     setNodes={setNodes}
